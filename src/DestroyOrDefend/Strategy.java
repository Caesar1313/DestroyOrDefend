...
public abstract class Strategy {
    ...
    public void InRangeEnemies() {
        ...
        if(IsInRange(Game.unitsInArena.get(i))  && force.PlayerType != Game.unitsInArena.get(i).PlayerType && force != Game.unitsInArena.get(i) && force.unit_type[Game.unitsInArena.get(i).type])
        ...
        {
            if(!IsInRange(EnemiesInRange.get(i))){
                EnemiesInRange.remove(EnemiesInRange.get(i));
        }
        }
...
class DefaultStrategy extends Strategy {
    ...
    public Forces ReturnFirstElement() {
        ...
        for (int i = 0; i < EnemiesInRange.size(); i++) {
            if (max == EnemiesInRange.get(i).health) {
                Forces temp = EnemiesInRange.get(i);
                EnemiesInRange.remove(i);
                return temp;
            }
...
class Customized extends Strategy{
    ...
    public int[] Priorities = new int[14];

    Customized(Forces f,int[] pr) {
        super(f);
        Priorities = pr;
    }
